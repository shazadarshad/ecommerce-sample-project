...
import { Filter, ShoppingCart, ArrowLeft, ChevronRight } from 'lucide-react';
...
export default function CategoryPage() {
  ...
  // Convert slug to category name (e.g., "electronics" -> "Electronics")
  const categoryName = categorySlug.charAt(0).toUpperCase() + categorySlug.slice(1);
  
  // Get all categories for validation
  const allCategories = Array.from(new Set(products.map((p) => p.category)));
  const isValidCategory = allCategories.some(cat => cat.toLowerCase() === categorySlug.toLowerCase());
        ...
        <div className="section-spacing relative overflow-hidden">
          <div className="max-w-7xl mx-auto container-padding relative z-10">
            ...
            >
              ...
              <ChevronRight className="w-4 h-4" />
              <span className="text-dark-navy dark:text-gray-100 font-semibold">
                {categoryName}
              </span>
            </motion.div>
            ...
            >
              ...
              <span className="font-semibold text-dark-navy dark:text-gray-100">{categoryProducts.length}</span> {categoryName.toLowerCase()} products
            </motion.div>
          </div>
        </div>
      </div>
